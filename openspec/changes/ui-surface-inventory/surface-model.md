...
| #1 and #4 (inert) | N/A — same component | #4 is a render branch *within* a #1 instance, not a separate surface. A leaf is either active (#1) or a bystander (#4), never both. |
| #6 (WriteBackModal) and #7 (RoutineReplaceModal) | No (in normal flow) | Both are app-modal and both are driven by the single-threaded store dispatch, at opposite ends of a session (see 2.4). #7 precedes a `start`; #6 follows a phase completion. They do not fire together. |
...
| 4 | A *different* routine is globally active (`status !== 'stopped'`) than this leaf's configured routine | The active routine matches this leaf's, or status returns to `stopped` | `timer-view.ts:99,120-122`; `routine-selection.ts:34-36` |
...
- There is one `EngineStore` (`main.ts:51`). A workspace-wide view (#9) would subscribe to the *same* store, exactly as `PomodoroTimerView` does (`timer-view.ts:31-34`). It cannot hold a second copy of timer state, so **#9 and #1 agree by construction** — there is no state to reconcile and no "do they need to agree" sync problem to solve. This directly answers `design.md` #9's own sub-question ("does starting a routine from the Bases view also populate the status bar"): yes, unavoidably, because both project the one store.
