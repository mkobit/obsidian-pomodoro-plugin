# file-mutation-apply Specification

## Purpose
TBD - created by archiving change file-mutation-apply-mechanism. Update Purpose after archive.
## Requirements
...
`applyMutations` SHALL stop dispatching further mutations as soon as one port call rejects. It SHALL resolve (never throw or reject) with `{ success: false, mutation, cause }`, where `mutation` is the failing mutation and `cause` is the original rejection. Mutations after the failing one in the input array SHALL NOT be dispatched to the port.
...
- **THEN** the port's method for the third mutation is never called, and `applyMutations` resolves with `{ success: false, mutation, cause }` referencing the second mutation

### Requirement: Empty batch is a no-op
`applyMutations` SHALL resolve immediately with `{ success: true }`, without invoking any port method, when called with an empty mutations array.

#### Scenario: Empty mutation list touches nothing
- **WHEN** `applyMutations` is called with an empty array
- **THEN** it resolves with `{ success: true }` without calling any `FileMutationPort` method
